...
const DEFAULT_COLORS = ['#2a5c4a', '#ff4704', '#ffb005', '#e8f5ef', '#15362b'];
const BAND_HALF = 17;
...
export function DiaTextReveal({
  ...
  useEffect(() => {
    ...
    // Re-measure when the text list identity changes.
    // eslint-disable-next-line react-hooks/exhaustive-deps -- join key tracks array content
    ...
    const controls = animate(sweepPos, SWEEP_END, {
      ...
      onComplete() {
        ...
        timerRef.current = setTimeout(() => {
          const next = (indexRef.current + 1) % lines.length;
  ...
  useEffect(() => {
    ...
    if (startOnView && !isInView) {
      // Leave readable solid text; replay sweep next time it enters view.
      stopRef.current?.();
